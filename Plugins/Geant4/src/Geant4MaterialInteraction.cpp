...
std::pair<double, double> Geant4MaterialInteraction::angleOfNormalVector(
    const Acts::Vector3D normalVector, const Acts::Vector3D momentum) const {
  ...
  // Check direction, invert direction if both vectors point in opposite
  // direction
  if (normalVector.dot(momentum) >= 0.)
    return std::make_pair(std::acos(normalVector.z() / normalVector.norm()),
                          tanphi);
  return std::make_pair(
      std::acos(normalVector.z() / normalVector.norm()) + M_PI, tanphi);
}
}
