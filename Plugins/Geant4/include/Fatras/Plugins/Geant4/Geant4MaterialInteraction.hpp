...
protected:
  /// @brief Calculates the angle of the normal vector for coordinate
  /// transformation
  ///
  /// @param [in] normalVector Normal vector of the penetrated material
  /// @param [in] momentum Momentum vector of the particle
  /// @return Pair consisting of Theta and Phi in spherical coordinates for the
  /// transformation
  std::pair<double, double>
  angleOfNormalVector(const Acts::Vector3D normalVector,
  ...
  G4ParticleDefinition *convertParticleToG4(const particle_t &particle) const;
  ...
  G4Material *convertMaterialToG4(const material_t &material) const;
...
template <typename particle_t>
G4ParticleDefinition *Geant4MaterialInteraction::convertParticleToG4(
  ...
  if (particle.pdg() != 0 && std::isfinite(particle.pdg())) {
    G4ParticleDefinition *parDef =
        m_particleTable->FindParticle(particle.pdg());
...
G4ParticleGun *Geant4MaterialInteraction::createParticleGun(
    const particle_t &particle, const std::pair<double, double> &angles) const {
  // Create particle
  ...
  if (parDef) {
    ...
    pGun->SetParticleMomentum(
        {particle.p() * std::sin(theta) * std::cos(phi) * scaleActsToG4,
...
template <typename material_t>
G4Material *Geant4MaterialInteraction::convertMaterialToG4(
    const material_t &material) const {
  // Translate material if valid
  if (material.Z() < 0. || material.A() < 0. || material.rho() < 0. ||
      !std::isfinite(material.Z()) || !std::isfinite(material.A()) ||
      !std::isfinite(material.rho()))
    return nullptr;
  return new G4Material("", material.Z(), material.A() * g / mole,
                        material.rho() * Acts::units::_cm * Acts::units::_cm *
                            Acts::units::_cm / Acts::units::_g * g / cm3);
}
...
void Geant4MaterialInteraction::convertParticlesFromG4(
  ...
  for (const MIparticle &bp : particlesG4) {
    // Correct angles
    p = bp.momentum.norm();
    theta = std::acos(bp.momentum.z() / p) -
            angles.first; // Modified due to symmetry of cos
    ...
    particle_t part(particleIn.position(), momentum,
...
operator()(particle_t &particle, const material_t &material,
  ...
  std::pair<double, double> angles =
      angleOfNormalVector(normalVector, particle.momentum());
