...
Fatras::PhotonConversionSampler::doConversion(
  ...
  // verbose output
  ACTS_VERBOSE("[ conv ] Child energy simulated as : " << childEnergy
                                                       << " MeV");
  ...
  if (N1 / (N1 + N2) < m_config.randomNumbers->draw(Fatras::Flat)) {
    ...
    for (;;) {
      ...
      if (F1 / F10 > m_config.randomNumbers->draw(Fatras::Flat))
        return m_config.childEnergyScaleFactor * epsilon;
    }
  } else {
...
Acts::Vector3D Fatras::PhotonConversionSampler::childDirection(
  ...
  ACTS_VERBOSE("[ conv ] Simulated angle to photon    = " << theta << ".");
...
Fatras::PhotonConversionSampler::getChildren(
  ...
  int pdg1 = Acts::convertToPdg(childType, charge1, false);
  int pdg2 = Acts::convertToPdg(childType, charge2, false);
