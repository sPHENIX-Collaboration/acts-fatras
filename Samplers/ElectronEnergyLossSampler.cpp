...
Fatras::ElectronEnergyLossSampler::ElectronEnergyLossSampler(
    const Fatras::ElectronEnergyLossSampler::Config& elConfig,
    std::unique_ptr<Acts::Logger> logger)
    : Fatras::IEnergyLossSampler(), m_config(), m_logger(std::move(logger)) {
  setConfiguration(elConfig);
}

Fatras::ElectronEnergyLossSampler::~ElectronEnergyLossSampler() {}

void Fatras::ElectronEnergyLossSampler::setConfiguration(
    const Fatras::ElectronEnergyLossSampler::Config& elConfig) {
  //!< @TODO update to configuration checking
  m_config = elConfig;
}

void Fatras::ElectronEnergyLossSampler::setLogger(
    std::unique_ptr<Acts::Logger> newLogger) {
  m_logger = std::move(newLogger);
}

Fatras::EnergyLoss Fatras::ElectronEnergyLossSampler::energyLoss(
    const Acts::MaterialProperties& materialProperties, double pInitial,
  ...
  double I = 16.e-6 * std::pow(materialProperties.averageZ(),
                               0.9);  // 16 eV * Z**0.9 - bring to MeV

  double delta = 0.;
  if (gamma > 10.) {
    double eplasma =
        28.816e-6 * sqrt(1000. * materialProperties.zOverAtimesRho());
  ...
  double MOP = -kazL * kaz * (log(2. * me * eta2 / I) + log(kazL / I) + 0.2 -
                              (beta * beta) - delta);

  double energyLossSigma =
      0.424 * 4. * kazL;  // 0.424: scale factor from sigma to FWHM

  double simulatedDeltaE =
      fabs(MOP) +
      energyLossSigma * m_config.randomNumbers->draw(Fatras::Landau);
  ...
  // @todo but this one is for reco, is it for sim as well?
  double u =
  ...
  simulatedDeltaE =
      simulatedDeltaE * simulatedDeltaE > E * E ? E : simulatedDeltaE;
  ...
  ACTS_VERBOSE(
      "[electron Eloss] created random deltaP as : " << sampledEloss.deltaE());
