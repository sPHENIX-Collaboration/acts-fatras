...
public:
  ...
  void
  setConfiguration(const Config& pcConfig);
...
protected:
  ...
  double
  childEnergyFraction(double gammaMom) const;

  Acts::Vector3D
  childDirection(const Acts::Vector3D& gammaMom, double childE) const;

  std::vector<Acts::InteractionVertex>
  getChildren(double                time,
  ...
  double
  phi1(double delta) const;

  /** helper functions for the Phi1/phi2 */
  double
  phi2(double delta) const;
...
inline double
PhotonConversionSampler::phi1(double delta) const
{
  if (delta <= 1.)
...
inline double
PhotonConversionSampler::phi2(double delta) const
{
