...
// constructor
Fatras::HadronicInteractionParametricSampler::
    HadronicInteractionParametricSampler(
        const Fatras::HadronicInteractionParametricSampler::Config& hiConfig,
...
void Fatras::HadronicInteractionParametricSampler::setConfiguration(
    const Fatras::HadronicInteractionParametricSampler::Config& hiConfig) {
  //!< @TODO update to configuration checking
  m_config = hiConfig;
}

void Fatras::HadronicInteractionParametricSampler::setLogger(
    std::unique_ptr<Acts::Logger> newLogger) {
  m_logger = std::move(newLogger);
}

std::vector<Acts::InteractionVertex>
Fatras::HadronicInteractionParametricSampler::doHadronicInteraction(
    double time, const Acts::Vector3D& position, const Acts::Vector3D& momentum,
    Acts::ParticleType particle) const {
  return getHadState(time, momentum.mag(), position, momentum.unit(), particle);
}

std::vector<Acts::InteractionVertex>
Fatras::HadronicInteractionParametricSampler::getHadState(
  ...
  ACTS_VERBOSE("[ had ] incoming particle energy | mass | momentum "
               << E << " | " << m << " | " << p << " | ");
  ...
  if (particle == Acts::pion || particle == Acts::kaon ||
      particle == Acts::pi0 || particle == Acts::k0) {
    pif = 0.15;
    nef = 0.25;
    prf = 0.25;
  }
  ...
  Acts::Vector3D ptemp(mom[0] * sin(th[0]) * cos(ph[0]),
                       mom[0] * sin(th[0]) * sin(ph[0]), mom[0] * cos(th[0]));
  double ptot = mom[0];
  ...
  for (int i = 1; i < Npart - 2; i++) {
    eps = 1. / (Npart - i);
    mom[i] = (eps + m_config.randomNumbers->draw(Fatras::Flat) * (1 - eps)) *
             (1 - ptot);
    if (ptemp.mag() < 1 - ptot) {
      while (fabs(ptemp.mag() - mom[i]) > 1 - ptot - mom[i]) {
        mom[i] =
            (eps + m_config.randomNumbers->draw(Fatras::Flat) * (1 - eps)) *
            (1 - ptot);
      }
    }
    // max p remaining
    ...
    double cthmax = fmin(
        1., (-ptemp.mag() * ptemp.mag() - mom[i] * mom[i] + p_rem * p_rem) / 2 /
                ptemp.mag() / mom[i]);
    double rnd = m_config.randomNumbers->draw(Fatras::Flat);
    theta = acos((cthmax + 1.) * rnd - 1.);
    phi = 2 * M_PI * m_config.randomNumbers->draw(Fatras::Flat);
    Acts::Vector3D test(sin(theta) * cos(phi), sin(theta) * sin(phi),
                        cos(theta));
    // create the rotation
  ...
  eps = 0.5;
  mom[Npart - 2] =
      pow(eps, m_config.randomNumbers->draw(Fatras::Flat)) * (1 - ptot);
  ...
  //   bv(p*particleDir.unit().x(),p*particleDir.unit().y(),p*particleDir.unit().z(),sqrt(etot*etot+p*p));
  ...
  //   for (int i=0; i<Npart; i++) {
  //     Acts::Vector3D
  //     dirCms(sin(th[i])*cos(ph[i]),sin(th[i])*sin(ph[i]),cos(th[i]));
  //     Acts::Vector3D childP = mom[i]*dirCms;
  //     in += childP;
  //     CLHEP::HepLorentzVector
  //     newp(childP.x(),childP.y(),childP.z(),sqrt(mom[i]*mom[i]+newm[i]*newm[i]));
  ...
  for (int i = 0; i < Npart; i++) {
    if (pdgid[i] < 10000) {
      ...
      if (childP.mag() > m_config.minimumHadOutEnergy) {
        ...
        pOutgoing.push_back(Acts::ParticleProperties(childP, part));
        numChildren++;
      }
  ...
  if (pOutgoing.size() > 0)
    children.push_back(
        Acts::InteractionVertex(vertex, time, m_config.processCode, pOutgoing));

  ACTS_VERBOSE("[ had ] it was kinematically possible to create "
               << gen_part << " shower particles and " << numChildren
