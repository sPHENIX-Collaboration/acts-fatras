...
#include "FatrasServices/MaterialInteractionEngine.h"
// Geometry module
...
DECLARE_SERVICE_FACTORY(Acts::MaterialInteractionEngine)

// constructor
Fatras::MaterialInteractionEngine::MaterialInteractionEngine(const std::string& name, ISvcLocator* svc):
  Acts::IMaterialEffectsEngine(),
  m_config()
//  randomNumbers("AtlasRandomNumberSvc", name),
//  m_doEnergyLoss(true),
//  m_energyLossSampler(""),
//  m_dedicatedElectronEnergyLoss(true),
//  m_elEnergyLossSampler(""),
//  m_minimumMomentum(50.),
//  m_createBremPhoton(true),
//  m_minimumBremPhotonMomentum(50.),
										 ...
										 const Acts::Material&) const
{
  ...
  if (m_doPositronAnnihilation and process == 5 ) {     // positron annihilation
    ...
    double fr = 1.-pow(fmin,randomNumbers->draw(Acts::Flat));
						 ...
						 float refX) const 
{
  ...
  while ( path < pathLim && p>m_minimumMomentum ) {
    ...
    double rndx = randomNumbers->draw(Acts::Flat);
    ...
    if ( path+dx > pathLim ) {
      double rndp = randomNumbers->draw(Acts::Flat);
                                                          ...
                                                          Acts::Vector3D& particleDir) const
{
  ...
  if (m_uniformHertzDipoleAngle) {
    ...
    theta = m/E * randomNumbers->draw(Acts::Flat); 
  } else {
    // ----->
    theta = m/E;
    // follow
