...
private:
  ...
  double nuclearInteractionProb(const double momentum, const double thickness,
                                const std::array<double, 6> &pars) const;
  ...
  double multiplicityProb(const double momentum, const double thickness,
                          const std::array<double, 7> &pars,
  ...
  unsigned int multiplicity(generator_t &generator, const double thickness,
                            particle_t &particle,
                            const std::array<double, 7> &pars) const;
  ...
  /// @tparam particle_t data type of the particle
  /// @param [in] generator random number generator
  /// @param [in] particleLookUp Look-up table for PID production probabilities
  /// to create
  ...
  std::vector<int>
  particleComposition(generator_t &generator,
                      const std::list<std::pair<double, int>> &particleLookUp,
                      const unsigned int nParticles) const;

  /// @brief Evaluates the fraction E_{out} / E_{in} of a single outgoing
  /// particle
  /// @note This function is based on the inverse sampling method and therefore
  /// requires a uniform distributed random number in [0,1].
  ///
  /// @param [in] cProb Uniform random number from cumulative probability
  /// distribution
  /// @param [in] scaling Fit parameter to scale the dependency on the
  /// multiplicity
  /// @param [in] n Demands the sampling from the distribution of the outgoing
  /// particle with the nth highest energy
  ///
  /// @return Energy fraction of the outgoing particle
  double energyFraction(const double cProb, const double scaling,
  ...
  std::vector<double> energyFractions(generator_t &generator,
  ...
  double cosThetaProbability(double cosTheta,
                             const std::array<double, 6> &fitParameters) const;

  /// @brief Sample a theta angle given a set of fit parameters for a certain
  /// model
  ///
  /// @tparam generator_t data type of the random number generator
  /// @param [in] generator random number generator
  ...
  /// @return Theta angle
  template <typename generator_t>
  double sampleTheta(generator_t &generator,
                     const std::array<double, 6> &fitParameters) const;
  ...
  std::vector<particle_t> kinematics(generator_t &generator,
                                     particle_t &particle,
  ...
  finalStateHadrons(generator_t &generator, const double thicknessInL0,
                    particle_t &particle, const parameters_t &params) const;
};

template <typename generator_t, typename particle_t>
unsigned int
ParametricNuclearInt::multiplicity(generator_t &generator,
                                   const double thickness, particle_t &particle,
                                   const std::array<double, 7> &pars) const {
...
template <typename generator_t>
std::vector<int> ParametricNuclearInt::particleComposition(
...
template <typename generator_t, typename parameters_t>
std::vector<double>
ParametricNuclearInt::energyFractions(generator_t &generator,
                                      const parameters_t &params,
                                      const unsigned int nParticles) const {
  ...
  while (true) {
    ...
    for (unsigned int n = 0; n < nParticles; n++) {
      if (nParticles <= 10) {
        ...
        // Extract the fit parameters to estimate the fit parameters for
        // sampling
        const std::pair<double, double> &fitParameters =
            params.energyScalingExtrapolation;
        eFraction = energyFraction(
            generator(), fitParameters.first + (n + 1) * fitParameters.second,
            n + 1);
      }
      result[n] = eFraction;
...
template <typename generator_t>
double ParametricNuclearInt::sampleTheta(
    generator_t &generator, const std::array<double, 6> &fitParameters) const {
...
template <typename generator_t, typename particle_t, typename parameters_t>
std::vector<particle_t> ParametricNuclearInt::finalStateHadrons(
    generator_t &generator, const double thickness, particle_t &particle,
    const parameters_t &params) const {
  // Calculate multiplicity
  const unsigned int Npart =
      multiplicity(generator, thickness, particle, params.multiplicity);
  ...
  const std::vector<int> particlePDGs =
      particleComposition(generator, params.particleTypes, Npart);
