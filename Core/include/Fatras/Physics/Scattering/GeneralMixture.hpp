...
struct GeneralMixture
{
      ...
      if (tob2 > 0.6 / std::pow(Z, 0.6)) {
        ...
        if (tob2 > 10) {
          scattering_params = getGaussian(
              particle.beta(), particle.p(), tInX0, genMixtureScalor);
        } else {
  ...
  std::array<double, 4>
  getGaussmix(double beta, double p, double tInX0, double Z, double scale) const
  {
    std::array<double, 4> scattering_params;
  ...
  std::array<double, 6>
  getSemigauss(double beta, double p, double tInX0, double Z, double scale)
      const
  {
  ...
  template <typename uniform_distribution_t, typename generator_t>
  double
  gaussmix(uniform_distribution_t&      udist,
  ...
  template <typename uniform_distribution_t, typename generator_t>
  double
  semigauss(uniform_distribution_t&      udist,
            generator_t&                 generator,
