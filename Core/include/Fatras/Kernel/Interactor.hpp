...
struct Interactor
{
  ...
  struct result_type;
  {
    /// Particle state after the simulation.
    Particle particle;
    /// Extra particles generated by interactions.
    std::vector<Particle> outgoing;
    /// Simulated hits created along the way.
             ...
             result_type&        result) const
  {
    ...
    // Update the stepper with the modified particle parameters
    stepper.update(state.stepping,
                   result.particle.position(),
                   result.particle.direction(),
                   result.particle.momentum(),
                   result.particle.time());
  }

  /// The interactor does not implement the observer interface.
  template <typename propagator_state_t, typename stepper_t>
  void
