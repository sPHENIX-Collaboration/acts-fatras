...
public:
  ...
  operator()(Generator& engine, const param_type& params)
  {
    ...
    double res = params.mean + quantile(x, params.scale);
    return res;
  }
...
private:
  /// Compute Landau quantile from probability.
  ///
  /// @param probability input probability wihthin [0,1)
  /// @param scale       Landau scale parameter
  static double
  quantile(double probablity, double scale);
